...
import csv
import re
from csv import Dialect, QUOTE_MINIMAL, register_dialect
...
def _field_extractor_function(field):
    ...
    if field.choices:
        try:
            return (lambda o: smart_str(getattr(o, 'get_%s_display' % field.name)()))
        except:
            return ""
    elif isinstance(field, BooleanField):
        return (lambda o: yesno(getattr(o, field.name), "Yes,No"))
    else:
        return (lambda o: smart_str(getattr(o, field.name)))
...
def export(request, app_label, model_name):
    ...
    mtm_fields = [f for f in model._meta.many_to_many if f.name in mtm_declared_fields]
    ...
    filters = {}
    for param_name, param_value in list(request.REQUEST.items()):
    ...
    for o in model_objects:
        try:
            ...
            row += [[smart_str(fv) for fv in getattr(o, om.name).all()] for om in mtm_fields]
            writer.writerow(row)
